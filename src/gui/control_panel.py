...
class ControlPanelSignals(QObject):
    """控制面板信号"""
    # 连接信号 (更新: 添加remember参数)
    connect_requested = pyqtSignal(str, bool)  # 参数: 房间号, 是否记住房间号
...
class ControlPanel(QWidget):
    ...
    def _init_ui(self):
        ...
        # 房间号将在 _load_saved_settings() 中加载
        room_layout.addWidget(room_label)
        room_layout.addWidget(self.room_input)
        connection_layout.addLayout(room_layout)

        # 记住房间号复选框
        self.remember_checkbox = QCheckBox("记住房间号")
        self.remember_checkbox.setChecked(True)  # 默认勾选
        self.remember_checkbox.setToolTip("下次启动时自动填充此房间号")
        connection_layout.addWidget(self.remember_checkbox)
        ...
        try:
            ...
            if self.gui_config.get_remember_room():
                ...
                if last_room:
                    ...
                    logger.info(f"已加载上次连接的房间号: {last_room}")

            # 设置复选框状态
            remember_room = self.gui_config.get_remember_room()
            self.remember_checkbox.setChecked(remember_room)

        except Exception as e:
            logger.warning(f"加载保存的设置失败: {e}")
    ...
    def _on_connect_clicked(self):
        ...
        if self.connect_button.isChecked():
            ...
            logger.info(f"请求连接房间: {room_id} (记住房间号: {remember})")
            self._is_connected = True
            self.connect_button.setText("断开")
            self.room_input.setEnabled(False)

            # 发射连接信号 (包含remember参数)
            self.signals.connect_requested.emit(room_id, remember)
        else:
            # 断开
            logger.info("请求断开连接")
            self._is_connected = False
